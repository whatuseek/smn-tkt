                                    ...
                                    <motion.div key={ticket._id} layout variants={cardVariants} initial="hidden" animate="visible" exit="exit" className={`font-raleway rounded-lg shadow-md p-4 flex flex-col justify-between relative border ${editingTicket === ticket._id ? (darkMode ? 'bg-gray-700/80 border-blue-500/50' : 'bg-blue-50/50 border-blue-300/70') : (darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200')} transition-colors duration-300 ease-in-out`} >
                                        ...
                                        <Box className={(isSaving === ticket._id || isDeleting === ticket._id || isUpdatingStatus === ticket._id) ? 'opacity-60' : ''}>
                                            ...
                                            {ticket.created_by_auth_id && (
                                                <div className="mb-1 text-xs text-gray-500 dark:text-gray-400 truncate" title={`Created by: ${ticket.created_by_auth_id}`}>
                                                    <strong>By:</strong> <span>{ticket.created_by_auth_id.substring(0, 8)}...</span>
                                                </div>
                                            )}

                                            {/* Edited Timestamp */}
                                            {/* Only show Edited if it's different from Created time, implying an actual edit happened */}
                                            {ticket.updatedAt && ticket.updatedAt !== ticket.createdAt && (
                                                <>
                                                    ...
                                                    {ticket.last_edited_by_auth_id && (
                                                        <div className="mb-2 text-xs text-gray-500 dark:text-gray-400 truncate" title={`Edited by: ${ticket.last_edited_by_auth_id}`}>
                                                            <strong>By:</strong> <span>{ticket.last_edited_by_auth_id.substring(0, 8)}...</span>
                                                        </div>
                                                    )}
                                                </>
                                            )}
